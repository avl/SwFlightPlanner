...
def fi_parse_airfield(icao):
    ...
    for pagenr in xrange(p.get_num_pages()):
        ...
        for item in page.get_by_regex("ATS COMMUNICATION FACILITIES"):
            lines=page.get_lines(page.get_partially_in_rect(0,item.y2+0.1,100,100))
            for line in lines:
                if line.count("RADIO NAVIGATION AND LANDING AIDS"): break
                twr=re.match(ur"TWR.*(\d{3}\.\d{3})",line)
                if twr:
                    ad['freqs'].append(('TWR',twr.groups()[0]))  
                atis=re.match(ur"ATIS.*(\d{3}\.\d{3})",line)
                if atis:
                    ad['freqs'].append(('ATIS',atis.groups()[0]))
...
def fi_parse_all_airfields():
    icaolist=[]
    icaolist.append("EFMA")
    ads=[]
    adspaces=[]
    for icao in icaolist:
        ad,adspaces=fi_parse_airfield(icao)
        ads.append(ad)
        adspaces.extend(adspaces)
    return ads,adspaces
