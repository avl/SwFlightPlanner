...
int WMM_InitializeGeoid(WMMtype_Geoid *Geoid)
  ...
  if (!swfproot)
    swfproot = ".";
  snprintf(fnamebuf, sizeof(fnamebuf), "%s/fplanquick/Magnetic2010/EGM9615.BIN", swfproot);

  if (( GeoidHeightFile = fopen( fnamebuf , "rb" ) ) == NULL)
  {
	WMM_Error(16);
...
*StartDate, WMMtype_Date *EndDate, int *ElementOption, int *PrintOption, char *OutputFile, WMMtype_Geoid *Geoid)
	...
	*/
	...
	if(Geoid->UseGeoid == 1)
		...
		fprintf(fileout, "Minimum Altitude above WGS-84 Ellipsoid: %lf\tMaximum Altitude above WGS-84 Ellipsoid: %lf\tStep Size: %lf\n", minimum->HeightAboveEllipsoid, maximum->HeightAboveEllipsoid, *a_step_size);
...
int  WMM_GetUserInput(WMMtype_MagneticModel *MagneticModel, WMMtype_Geoid *Geoid, WMMtype_CoordGeodetic *CoordGeodetic, WMMtype_Date *MagneticDate)
	...
	*/
	...
	for(i = 0, done = 0; i <= 40 && !done; i++)
	{
		...
		if(done)
		{
			if(MagneticDate->DecimalYear > MagneticModel->epoch + EPOCHRANGE ||MagneticDate->DecimalYear < MagneticModel->epoch)
			{
...
int WMM_Warnings(int control, double value, WMMtype_MagneticModel *MagneticModel)
	...
	switch(control)
	{
		...
		case 4:/*Date outside the recommended range*/
			...
			printf("\n VALID RANGE  = %d - %d\n" , (int)MagneticModel->epoch, (int)MagneticModel->epoch + EPOCHRANGE);
			printf(" TIME   = %lf\n", value);
			while(1)
			{
